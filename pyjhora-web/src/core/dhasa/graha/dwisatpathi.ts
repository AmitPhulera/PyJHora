 ...
 */
export async function dwisatpathiDashaStartAsync(
  jd: number,
  place: Place,
  starPositionFromMoon = 1,
  seedStar = 19,
 ...
 * Async version: supports special starting planets (M/G/B/I/P/T)
 */
export async function getDwisatpathiDashaBhuktiAsync(
  jd: number,
  place: Place,
  options: {
    starPositionFromMoon?: number;
  ...
  const [initialLord, initialStartJd] = await dwisatpathiDashaStartAsync(
  ...
  for (let cycle = 0; cycle < dhasaCycles; cycle++) {
    for (let i = 0; i < 8; i++) {
      ...
      if (includeBhuktis) {
        let bhuktiLord = currentLord;
        if (antardashaOption === 3 || antardashaOption === 4) bhuktiLord = getNextDwisatpathiLord(bhuktiLord, 1);
        else if (antardashaOption === 5 || antardashaOption === 6) bhuktiLord = getNextDwisatpathiLord(bhuktiLord, -1);
        const direction = (antardashaOption === 1 || antardashaOption === 3 || antardashaOption === 5) ? 1 : -1;
        const bhuktiDuration = durationYears / 8;
        let bhuktiStartJd = startJd;
        for (let j = 0; j < 8; j++) {
