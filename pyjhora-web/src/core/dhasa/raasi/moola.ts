/**
 * Moola Dasha System
 * Ported from PyJHora moola.py
 * 
 * Also called Lagna Kendradi Rasi Dhasa (alternate name)
 * Very similar to Kendradhi - uses kendras from stronger of Asc/7th
 */
...
export interface MoolaDashaPeriod {
  rasi: number;
  rasiName: string;
  startJd: number;
  startDate: string;
...
export interface MoolaBhuktiPeriod {
  dashaRasi: number;
  bhuktiRasi: number;
...
export interface MoolaResult {
  mahadashas: MoolaDashaPeriod[];
  bhuktis?: MoolaBhuktiPeriod[];
}
...
export function getMoolaDashaBhukti(
  ...
  // Build kendra progression
  const dhasaProgression = KENDRAS.map(k => 
    (dhasaSeedSign + direction * (k - 1) + 12) % 12
  );
  
  const mahadashas: MoolaDashaPeriod[] = [];
  const bhuktis: MoolaBhuktiPeriod[] = [];
  let startJd = jd;
  let totalDuration = 0;
